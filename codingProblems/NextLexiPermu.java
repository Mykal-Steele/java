...
public class NextLexiPermu {
    ...
    public static void main(String[] args) {
        ...
        if (maxLexi) {
            ...
            // HashMap to store potential swap candidates and their differences from pivot
            // element
            ...
            for (int i = pivotIndex; i < arr.length; i++) {
                ...
                if (i + 1 < arr.length && arr[i + 1] > arr[i]) {
                    hm.put(arr[i + 1], arr[i + 1] - arr[i]); // Store the difference to help find the smallest greater
                                                             // element
                }
            }

            // Find the minimum difference element greater than pivot (closest larger
            // element)
